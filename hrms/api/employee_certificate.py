...
@frappe.whitelist()
def get_employee_certificate_requests(
    employee: str,
    for_approval: bool = False,
    limit: int | None = None,
) -> list[dict]:
    """Return recent Employee Certificate requests for an employee.

    Mirrors the structure of get_leave_resumption_requests
    so the frontend can reuse common list/details UIs.
    """

    filters: dict[str, object] = {"employee": employee}

    # For parity with other APIs (future-proof if approver is added)
    if for_approval:
    ...
    try:
        workflow_state_field = frappe.get_meta("Employee Certificate").get_field(
            "workflow_state"
        )
