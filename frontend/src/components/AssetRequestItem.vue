		...
		<template #left>
			<div class="flex flex-col items-start gap-1.5">
				<div class="text-base font-normal text-gray-800 truncate max-w-[150px]">
					{{ props.doc.requested_item_name }}
				</div>
				<div class="text-xs font-normal text-gray-500 truncate max-w-[150px]">
					<span>{{ props.doc.request_date }}</span>
					<span v-if="props.doc.reason_for_request">
						<span class="whitespace-pre"> &middot; </span>
						<span class="whitespace-nowrap">{{ props.doc.reason_for_request.substring(0, 30) }}{{ props.doc.reason_for_request.length > 30 ? '...' : '' }}</span>
					</span>
				</div>
			</div>
		</template>
