		...
		<template #left>
			<div class="flex flex-col items-start gap-1.5">
				<div class="text-base font-normal text-gray-800 truncate max-w-[150px]">
					{{ props.doc.purpose || __("Employee Certificate") }}
				</div>
				<div class="text-xs font-normal text-gray-500 truncate max-w-[150px]">
					<span>{{ props.doc.certificate_date }}</span>
					<span v-if="props.doc.reason">
						<span class="whitespace-pre"> &middot; </span>
						<span class="whitespace-nowrap">{{ props.doc.reason.substring(0, 30) }}{{ props.doc.reason.length > 30 ? '...' : '' }}</span>
					</span>
				</div>
			</div>
		</template>
